...
class FoodAdmin(SummernoteModelAdmin):
    ...
    **Fields**
    ``list_display``
        Fields to display in the admin list view: `food_name`, `food_type`, `price`, and `available`.
    ``search_fields``
        Fields that can be searched: `food_name` and `description`.
    ``list_filter``
        Filters available in the admin list view: `available` and `food_type`.
    ``summernote_fields``
        Specifies the fields to be rendered with the Summernote rich-text editor: `description`.
...
class DrinkAdmin(SummernoteModelAdmin):
    ...
    **Fields**
    ``list_display``
        Fields to display in the admin list view: `drink_name`, `drink_type`, `price`, and `available`.
    ``search_fields``
